...
const SecurityPage = () => {
    ...
    const BASE_URL="http://localhost:1243/compo/addCompo"
    const GETT_URL="http://localhost:1243/portfolio/fetchPortfolioByName"
    const FETCHH_URL="http://localhost:1243/compo/fetchByName"
    const getDataBySym="http://localhost:1243/master/fetchDataBySymbol"
    const getAssetBytheme="http://localhost:1243/themeAsset/getByTheme"
    ...
    function addCompo(pName,urlValue,portfolioComposition)
    {
        return axios.post(`${BASE_URL}/`+pName+`/`+urlValue,portfolioComposition);
    }
    ...
    function GetByName(){
        findByName(pName).then(
            (response)=>{setPortfolioDatas(response.data)
                        console.log(response.data) }
    ).catch(error=>{console.log(error);} )
    }
    ...
    useEffect(()=>{
       ...
       useEffect(()=>{findByName(pName).then(
        (response)=>{setPortfolioDatas(response.data)
                    //console.log(response.data) 
        ...
        const selectSymbol=(e)=>{
            ...
            {getDataBySymbol(e.target.value).then(
                (response)=>{setsymbolData(response.data)
                            console.log(response.data) 
            ...
            event.preventDefault()
              const portfolioComposition={equityCategory,exchangeName,units,pName,symbol}
                    ...
                    {setPrlValue(portfolioComposition.portfolioName)};
                        addCompo(pName,urlValue,portfolioComposition).then(
                ...
                <div>
                    ...
                    <div className="secpages">
                        <div className="secpage-1">
                            <span className="secpage-asset">Asset:</span>&nbsp;
                            ...
                            <option value="" selected disabled >--Select Your Choice--</option>
                                    {
                                        assetData? (assetData.map((data,key)=>(
                                                <option value={data.assetId}> {data.assetClass}</option>
                                            ))):(<>Loading...</>)
                                    }
                            </select>
                        </div>
                        <div className="secpage-2">
                            <span className="secpage-price" >Price:</span>&nbsp;
                            <input style={{"color":"black"}} value={(symbolData.lastPrice)}></input>
                               &nbsp;
                        </div>
                        <div className="secpage-3">
                            <span className="secpage-units">Units:</span>&nbsp;
                            <input className="secpage-units-input" typt="number" placeholder="Enter units" value={units} onChange={calculateValue}></input>&nbsp;
                        </div>
                        <div className="secpage-4">
                            <span className="secpage-sec">Search Securities:</span>&nbsp;
                        ...
                        <div className="secpage-5">
                            <span className="secpage-equity">Equity Category:</span>&nbsp;
                        ...
                        <div className="secpage-6">
                            <span className="secpage-exchange">Exchange:</span>&nbsp;
